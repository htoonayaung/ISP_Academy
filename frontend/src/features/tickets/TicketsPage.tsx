...
export function TicketsPage() {
  ...
  async function setStatus(ticket: Ticket, status: "DRAFT" | "PUBLISHED" | "ARCHIVED", confirmation: string) {
    if (!confirm(confirmation)) return;
    try {
      setError("");
      if (status === "PUBLISHED") await api(`/api/v1/tickets/${ticket.id}/publish`, { method: "POST" });
      else if (status === "ARCHIVED") await api(`/api/v1/tickets/${ticket.id}/archive`, { method: "POST" });
      else await api(`/api/v1/tickets/${ticket.id}`, { method: "PATCH", bodyJson: { status: "DRAFT" } });
      await load();
